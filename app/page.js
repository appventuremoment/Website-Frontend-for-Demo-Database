...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated'){
      setFrontLogoText(  <>
        ...
        <br/>
        <span>Welcome to Project DB</span>
      </>
      );
    }
  ...
  function AutoResizeFont(minFontSize, maxFontSize) {
    ...
    }, [minFontSize, maxFontSize, frontLogoText]);
  
    return ref;
  }

  function AutoResizeHeader(minFontSize, maxFontSize) {
    const ref = useRef(null);
    ...
    useEffect(() => {
      ...
      const resizeText = () => {  
        ...
        while (elem.scrollWidth > window.innerWidth * 0.425 && fontSize > minFontSize) {
          fontSize = fontSize - 0.1;
          elem.style.fontSize = `${fontSize}vw`;
        }
      };
  ...
  const headerRef1 = AutoResizeHeader(6, 10.5);
  const headerRef2 = AutoResizeHeader(6, 10.5);
  return (
    <main style={{ overflow: 'hidden'}}>
      <div id='div-1'>
        <b id='front-logo' ref={textRef} className='fade-in'>{frontLogoText}</b>
